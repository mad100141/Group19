...
```{r}
#GDP (current LCU)
Variables <- merge(Countries, filter(indicators, IndicatorCode=="NY.GDP.MKTP.CN" &  Year==2012 & Value > 1e+3), by = 'CountryCode')
names(Variables)[names(Variables)=="Value"] <- "GDP"

#Mortality rate, under-5 (per 1,000)
Variables <- merge(Variables, filter(indicators, IndicatorCode=="EG.USE.PCAP.KG.OE" &  Year==2012), by = 'CountryCode')
names(Variables)[names(Variables)=="Value"] <- "Mortality"

GDPMortality <- Variables

#Central government debt, total (% of GDP)
Variables2 <- merge(Countries, filter(indicators, IndicatorCode=="GC.TAX.INTT.CN" &  Year==2012), by = 'CountryCode')
names(Variables2)[names(Variables2)=="Value"] <- "Debt"

#Military expenditure (% of GDP)
Variables2 <- merge(Variables2, filter(indicators, IndicatorCode=="MS.MIL.XPND.GD.ZS" &  Year==2012), by = 'CountryCode')
names(Variables2)[names(Variables2)=="Value"] <- "Military"
...
ggplot(GDPMortality, aes(x = GDP, y = Mortality)) + geom_point() + 
  labs(
...
ggplot(Variables2, aes(x = Military, y = Debt)) + geom_point() + 
  labs(
    title = "Military Expenditure vs Government Debt",
    x = "GDP (current LCU)",
    y = "Mortality rate, under-5 (per 1,000)"
  )
