...
ggplot(GDPMortality, aes(x = GDP ^ (1/4), y = Mortality, label = ShortName)) + geom_point() + 
  labs(
    title = "GDP vs Primary Education Survival Rate",
    x = "GDP (current US Dollar)",
    y = "Survival rate to the last grade of primary education, both sexes (%)"
  ) + geom_smooth() +
  geom_text(data=subset(GDPMortality, GDP ^ (1/4) > 5000))


ggplot(Variables2, aes(x = Expense, y = Debt, label = ShortName)) + geom_point() + 
  labs(
    title = "Government Expenses vs Government Debt",
    x = "Expenses (% of GDP)",
    y = "Central government debt, total (% of GDP)"
  ) + geom_smooth() +
  geom_text(data=subset(Variables2, Debt > 150))
