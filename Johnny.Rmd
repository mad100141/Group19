...
Variables <- merge(Variables, filter(indicators, IndicatorCode=="SH.MED.PHYS.ZS" &  Year==2012), by = 'CountryCode')
...
#Expense (% of GDP)
Variables2 <- merge(Countries, filter(indicators, IndicatorCode=="GC.XPN.TOTL.GD.ZS" &  Year==2012), by = 'CountryCode')
...
#actually GDP^1/4 vs num of physicians
ggplot(GDPMortality, aes(x = GDP ^ (1/4), y = Mortality)) + geom_point() + 
  labs(
    title = "GDP vs Physician Count",
    x = "GDP (current US Dollar)",
    y = "Physicians (per 1,000 people)"
  ) + geom_smooth()


ggplot(Variables2, aes(x = Debt, y = Expense)) + geom_point() + 
  labs(
