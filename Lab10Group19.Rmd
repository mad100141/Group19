...
The World Development Indicators from the World Bank contains over a 
thousand annual indicators of economic development from hundreds of 
countries around the world.

The World Development Indicators is a dataset consisting of 247 countries and 
1,344 indicator variables. It is impossible to specify what each observation 
corresponds to because there are so many but several categories observed are 
economic measures like agricultural and industrial measures for land or CO2 emmisions and 
more general measures like life expectancy, labor participation, literacy rates, 
mortality rates, and income disparity.
...
1. What variables have the strongest correlations with literacy rates? 
  (variables like GDP, population, mortality rate, employment rate, etc)
  Correlation Graph
...
Other functions include bmp, jpeg, and tiff.

(b)  How do you control the height and width of images with the `pdf()` function?  What are the units for these parameters?

There are width and height parameters in the function that can be used to control the values. The units are inches, with a default value of 7.
...
There are also height and width parameters in the png function. The default units is Pixels.

(d)  What types of images can you save with the `ggsave()` function?  (Hint:  Look at the `device` parameter.)

ggsave() can save eps/ps, tex (pictex), pdf, jpeg, tiff, png, bmp, svg and wmf. 

(e)  What does the `dpi` parameter control in the `ggsave()` function?  What does `dpi` stand for?

DPI stands for dots per inch. It is used to raster graphics. 
